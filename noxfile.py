...
ALWAYS_USE_PIP = [
    "pylint-junit",
    # TODO unlearn: place any pip-only dependencies here and remove the following
    "pandera",
    # tables not required, but we test https://unlearnai.atlassian.net/browse/DP-222
    "tables",
...
def _preprocess_version(version: str = "") -> str:
...
def build(session: Session) -> None:
    ...
    version = "" if not session.posargs else session.posargs[0]
    ...
    setup_version = read_configuration("setup.cfg")["metadata"]["version"]
    assert setup_version.endswith(
        "+git"
    ), "setup.cfg version should end with `+git`"
    assert setup_version.rstrip("+git") == version, (
        f"setup.cfg version {setup_version} does not match "
    ...
    session.run(
        ...
        *userargs,
        "--repository",
        "pypi-unlearnai",  # must match entries in ~/.pypirc
        f"dist/{PACKAGE}-{version}*",
    )
