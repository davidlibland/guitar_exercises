...
def main(conda_fname: str, pip_fname: str, compare: bool = False) -> bool:
    """
    ...
    """
    with open(conda_fname, encoding="utf-8") as conda_fd:
    ...
    header = (
        f"# This file is auto-generated from {os.path.split(conda_fname)[1]}, do not modify.\n"
        "# See that file for comments about the need/usage of each dependency.\n\n"
    )
    pip_content = header + "\n".join(pip_deps)

    if compare:
        with open(pip_fname, encoding="utf-8") as pip_fd:
            lhs = pip_content.splitlines(keepends=False)
            rhs = pip_fd.read().splitlines(keepends=False)
            diff = list(difflib.unified_diff(lhs, rhs))

            if len(diff) > 0:
                print("Comparison between the regenerated `requirements-dev.txt` "
        ...
        with open(pip_fname, "w", encoding="utf-8") as pip_fd:
